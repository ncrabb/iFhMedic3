...
@protocol DismissAllergyDelegate <NSObject>

-(void)dismissViewControl;
-(void)dismissViewControlAndStartNew:(NSInteger)tag;
@end

@interface AllergyViewController : UIViewController <DismissValidateControl>
{
    __weak id <DismissAllergyDelegate> delegate;
}
