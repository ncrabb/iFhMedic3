...
-(void) setViewUI:(NSInteger)page
{
    ...
    NSString* sqlGroup = @"SELECT count (distinct InputGroup) FROM inputs where inputpage = 'Allergies'";
    ...
    {
        NSString* querySql = @"select InputID, InputName, InputDataType, InputGroup, InputRequiredField  from Inputs where InputPage = 'Allergies' order by inputGroup, InputIndex";
        @synchronized(g_SYNCLOOKUPDB)
        {
...
- (void) loadDefault
{
    NSString* sql = @"Select inputID, 'Inputs', InputDefault from Inputs where inputPage like 'Allergies' and (inputDefault != '')";
