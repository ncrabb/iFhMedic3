...
-(void) setViewUI:(NSInteger)page
{
    ...
    NSString* sqlGroup = @"SELECT count (distinct InputGroup) FROM inputs where inputpage = 'CPR'";
    ...
    {
        NSString* querySql = @"select InputID, InputName, InputDataType, InputGroup, InputRequiredField  from Inputs where InputPage = 'CPR' order by inputGroup, InputIndex";
        @synchronized(g_SYNCLOOKUPDB)
        {
...
- (void) loadDefault
{
    NSString* sql = @"Select inputID, 'Inputs', InputDefault from Inputs where inputPage like 'CPR' and (inputDefault != '')";
