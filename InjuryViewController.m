...
-(void) setViewUI:(NSInteger)page
{
    ...
    NSString* sqlGroup = @"SELECT count (distinct InputGroup) FROM inputs where inputpage = 'Injury'";
    ...
    {
        NSString* querySql = @"select InputID, InputName, InputDataType, InputGroup, InputRequiredField  from Inputs where InputPage = 'Injury' order by inputGroup, InputIndex";
        @synchronized(g_SYNCLOOKUPDB)
        {
...
- (void) loadDefault
{
    NSString* sql = @"Select inputID, 'Inputs', InputDefault from Inputs where inputPage like 'Injury' and (inputDefault != '')";
