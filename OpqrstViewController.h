...
@protocol DismissOpqrstDelegate <NSObject>

-(void)dismissViewControl;
-(void)dismissViewControlAndStartNew:(NSInteger)tag;
@end

@interface OpqrstViewController : UIViewController <DismissValidateControl>
{
    __weak id <DismissOpqrstDelegate> delegate;
}
