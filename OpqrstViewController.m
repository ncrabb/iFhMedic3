...
@interface OpqrstViewController ()
{
...
@implementation OpqrstViewController
...
-(void) setViewUI:(NSInteger)page
{
    ...
    NSString* sqlGroup = @"SELECT count (distinct InputGroup) FROM inputs where inputpage = 'OPQRST'";
    ...
    {
        NSString* querySql = @"select InputID, InputName, InputDataType, InputGroup, InputRequiredField  from Inputs where InputPage = 'OPQRST' order by inputGroup, InputIndex";
        @synchronized(g_SYNCLOOKUPDB)
        {
...
- (void) loadDefault
{
    NSString* sql = @"Select inputID, 'Inputs', InputDefault from Inputs where inputPage like 'OPQRST' and (inputDefault != '')";
