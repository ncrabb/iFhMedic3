...
@protocol DismissSymptomDelegate <NSObject>

-(void)dismissViewControl;
-(void)dismissViewControlAndStartNew:(NSInteger)tag;
@end

@interface SymptomViewController : UIViewController <DismissValidateControl>
{
    __weak id <DismissSymptomDelegate> delegate;
}
